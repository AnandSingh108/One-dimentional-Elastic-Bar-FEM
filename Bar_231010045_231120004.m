...
AE    = [1 1] ; %Linear Variation of AE 1+2*x
C     = [0 0 0]; %quadratic variation of distributed springs 1+2*x+x^2
T     = [0 1 0 0]; %quadratic variation of distributes tractions 1+2*x+x^2+sin(pi*x/L)
...
for P=1:length(NELEM_array)
    ...
    for i=1:NELEM+1
        X_node(i)=(i-1)*h_I; % x cordinates of the nodes
    end
...
function I=Integrate(f,A,B,n)
    ...
    %changing inegration
    for i=1:n
        t=@(x) (diff)/2*x+(A+i*diff-diff/2); % transformation from A B to -1,1
